...
interface NavItem {
  ...
  icon: IconName;
  to: string;
  badgeKey?: "exercise" | "feedback" | "chat";
}
...
export function StudentSidebar() {
  ...
  const { unreadMessages, newFeedback, pendingExercises } = useNotifications();
  const badgeForKey = (key?: NavItem["badgeKey"]) => {
    ...
    <aside className="hidden md:flex w-[248px] bg-ink text-paper flex-col border-r border-line-dark min-h-screen">
      ...
      <div className="px-4 py-4 border-t border-line-dark">
        ...
        >
          ...
          <Icon name="chevron" size={12} className="text-[#9E8E82] group-hover:text-paper transition" />
        </Link>
        <button
          onClick={signOut}
